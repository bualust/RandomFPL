...
def generate_team():
    ...
    for plot,title in to_plot.items():
        fig, ax = plt.subplots()
        max_val = int(max(df[plot]))
        min_val = int(min(df[plot]))
        number_of_bins = 10
        ...
        plt.hist(forwards[plot], histtype='step', label="Forwards", density=True, bins=bins)
        midfields = df[df['Position']==3]
        plt.hist(midfields[plot], histtype='step', label="Midfielders", density=True, bins=bins)
        plt.hist(random_team[plot], histtype='step', label="Random Penguins", density=True, bins=bins)
        ax.legend()
...
def print_average_quantities(df):
    ...
    forwards    = forwards[forwards["Minutes"]!=0]
    midfielders = midfielders[midfielders["Minutes"]!=0]
    defender    = defender[defender["Minutes"]!=0]
...
def get_random_team(df, isFirstAttempt, random_team, av_g_f, av_a_m, av_g_c):
    ...
    if isFirstAttempt==True:
        ...
        print('--- and welcome our new team member')
        print(new_member)
...
def maximise_expense(random_team, df, av_g_f,av_a_m,av_g_c):
    ...
    while expense < 102.1:
        ...
        duplicate = new_random_team[new_random_team.duplicated()]
